...
FUNCTION querySoftwareTitleFromSW($_id){
...
if (isset($_POST["Submit"])) {
    ...
    if ($_POST['email'] != "") {
        ...
        if(formCheckMandatoryFields($DEBUG)){
            ...
            #@@@
            echo "@@ checkMandatoryFormField UNIT TEST done<br>";
        }
        ...
        for ($n=0;$n < $authorNo = 5;$n++){
            ...
            if($gn && $fn){
                #FUNCTION CALL
                $arrayAuthorIDs[$n] = queryAuthorIDFromAuthorByName($gn,$fn,$DEBUG);
        ...
        #6 Publisher
        $publisherID = queryPublisherID($publisher, $DEBUG); # 1 for verbose output
    ...
    if ($insertionStatus == 0 && $fileUploadStatus 	== 0 &&
        ...
        for($n=0;$n < $authorNo; $n++){
            ...
            if($arrayAuthorIDs[$n] != 0){ #ie if existing author -> insert to author table, then author-software table
                ...
                #FUNCTION CALL
                insertAuthorsToAuthorSoftwareByIDArray( $arrayAuthorIDs[$n], $gn, $fn, $softwareID, 1);
            }
        } #end loop
